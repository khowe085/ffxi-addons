...
Echo displays a user-provided string in a persistent, draggable on-screen text overlay. The text is set via command and its position is configured through the standard `setup`/`exit` GUI flow, with both the text and position persisted per character.
...
| Command            | Description                                                                                          |
|--------------------|------------------------------------------------------------------------------------------------------|
| `//ec set <text>`  | Set and display the text. Saved immediately; if setup is open, staged until exit. Multi-word text is supported. With no text, prints the command list. |
| `//ec clear`       | Clear the displayed text.                                                                             |
| `//ec setup`       | Open the positioning GUI; the overlay becomes draggable.                                              |
| `//ec exit`        | Save staged position changes and close setup.                                                         |
...
Position is changed by dragging the overlay during `//ec setup` and is only persisted on `//ec exit` (save); `//ec exit -d` reverts to the pre-setup position. The `text` value set with `//ec set` is persisted immediately when not in setup.
