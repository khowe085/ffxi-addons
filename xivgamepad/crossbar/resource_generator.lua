-- PORT: from xivcrossbar/resource_generator.lua (xivgamepad crossbar port;
-- see .planning/xivgamepad-contracts.md, "crossbar/ subtree rule").
-- Edits:
--   1. require paths: 'libs/kebab_casify', 'libs/ordered_pairs' and
--      'libs/md5' -> 'crossbar/...' (subtree relocation).
--   2. output path redirection: 'resources/crossbar_spells.lua' and
--      'resources/crossbar_abilities.lua' -> 'data/generated/...' (runtime
--      files live under data/ only).
--   3. get_crossbar_spells_hash / get_crossbar_abilities_hash: replaced
--      require('resources/crossbar_*') with files.read + loadstring inside
--      pcall -- require would serve a stale cached module after a
--      regeneration, and tests must stay on the in-memory files mock.
--      Missing/corrupt/non-table content yields nil so callers regenerate.
-- No windower event registrations; the only require-time effect is
-- constructing the two (now data/generated/) file handles below -- the
-- upstream `true` flag to files.new performs no I/O at construction, only
-- deferring file creation to the first write, so nothing touches the disk
-- before gamedata's create_dir.
-- The '../../res/*.lua' reads are unchanged upstream code: the documented
-- read-only walk-up exception for MD5 freshness against Windower's res
-- sources (the never-walk-above-addon_path rule governs creation/writes).

local res = require 'resources'
local kebab_casify = require('crossbar/kebab_casify')
local files = require('files')
local ordered_pairs = require('crossbar/ordered_pairs')
local md5 = require('crossbar/md5')

local spells_dot_lua = files.new('data/generated/crossbar_spells.lua', true)
local abilities_dot_lua = files.new('data/generated/crossbar_abilities.lua', true)
...
local get_crossbar_spells_hash = function()
    local content = files.read(spells_dot_lua)
    local ok, crossbar_spells = pcall(function() return loadstring(content)() end)
    if (not ok or type(crossbar_spells) ~= 'table') then
        return nil
    else
...
local get_crossbar_abilities_hash = function()
    local content = files.read(abilities_dot_lua)
    local ok, crossbar_abilities = pcall(function() return loadstring(content)() end)
    if (not ok or type(crossbar_abilities) ~= 'table') then
        return nil
    else
